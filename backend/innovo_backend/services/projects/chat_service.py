...
from innovo_backend.shared.models import Project, ProjectContext, ProjectChatMessage
...
def _build_system_prompt(ctx: ProjectContext | None) -> str:
    ...
    if ctx.company_profile_json:
        try:
            profile = json.loads(ctx.company_profile_json) if isinstance(ctx.company_profile_json, str) else ctx.company_profile_json
    ...
    if ctx.funding_rules_json:
        try:
            rules = json.loads(ctx.funding_rules_json) if isinstance(ctx.funding_rules_json, str) else ctx.funding_rules_json
    ...
    if ctx.domain_research_json:
        try:
            research = json.loads(ctx.domain_research_json) if isinstance(ctx.domain_research_json, str) else ctx.domain_research_json
    ...
    if ctx.retrieved_examples_json:
        try:
            examples = json.loads(ctx.retrieved_examples_json) if isinstance(ctx.retrieved_examples_json, str) else ctx.retrieved_examples_json
    ...
    if ctx.style_profile_json:
        try:
            style = json.loads(ctx.style_profile_json) if isinstance(ctx.style_profile_json, str) else ctx.style_profile_json
...
def _merge_company_corrections(ctx: ProjectContext, corrections: dict, db: Session) -> None:
    ...
    if ctx.company_profile_json:
        try:
            profile = json.loads(ctx.company_profile_json) if isinstance(ctx.company_profile_json, str) else dict(ctx.company_profile_json)
        except (json.JSONDecodeError, TypeError):
            profile = {}
